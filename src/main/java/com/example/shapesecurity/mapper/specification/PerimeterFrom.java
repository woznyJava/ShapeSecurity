...
public class PerimeterFrom implements ShapeSpecification{
    ...
    public Specification<Shape> toSpecification(FilterRequest filterRequest) {
        return filterRequest.getMap().get("perimeterFrom") != null ? (root, query, cb) -> cb.greaterThanOrEqualTo(root.get("perimeter"), Double.parseDouble(filterRequest.getMap().get("perimeterFrom").toString())) : null;
    }
