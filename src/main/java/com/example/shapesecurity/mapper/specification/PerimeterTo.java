...
public class PerimeterTo implements ShapeSpecification{
    ...
    public Specification<Shape> toSpecification(FilterRequest filterRequest) {
        return filterRequest.getMap().get("perimeterTo") != null ? (root, query, cb) -> cb.lessThanOrEqualTo(root.get("perimeter"), Double.parseDouble(filterRequest.getMap().get("perimeterTo").toString()) ) : null;
    }
